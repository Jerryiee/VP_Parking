...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                global prev_positions, cars_in, cars_out
                #read database file
                ...
                if(detection_lines is not None and detection_ref_lines is not None):
                    ...
                    for i in range(len(detection)):
                        ...
                        if id in prev_positions:
                            prev_cx, prev_cy = prev_positions[id]
                             #dist error
                            dist = distanceCalculate((cx, cy), (prev_cx, prev_cy))
                            if dist < 50:
                    ...
                    cv2.putText(im0, str(cars_in), (110, 400), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0,255,0), 2, cv2.LINE_AA)# show count
                    cv2.putText(im0, str(cars_out), (110, 500), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0,0,255), 2, cv2.LINE_AA)# show count
                    ...
                    #cv2.putText(im0, str(cars_in), (sx1-5, sy1), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 1, cv2.LINE_AA)# show count
                    #cv2.putText(im0, str(cars_out), (sxr1-5, syr1), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 1, cv2.LINE_AA)# show count
...
run(source="traffic.mp4", weights="yolov5s.pt", conf_thres=0.25, imgsz=[640, 640], classes= cars, view_img=True, line_thickness=2, nosave= True, device = 0) 
"""
def main(opt):
